...
class InvertedPendulum(object):
    def __init__(
        ...
        if add_noise_to_gravity_and_mass:
            ...
            self.masspole = 0.6 + np.random.uniform(-0.05, 0.2)
        else:
        ...
        self.theta_threshold_radians = 180 * math.pi / 360  # pi/2
...
class InvertedPendulumGame(object):
    def __init__(
        ...
        self.pendulum = InvertedPendulum(
            windowdims, cartdims, penddims, gravity, add_noise_to_gravity_and_mass
        )
    ...
    def game_round(self):
        ...
        for i in range(self.max_timestep):
            ...
            if self.PID_controller is None:
                ...
                action = self.PID_controller.get_action(
                    self.pendulum.get_state(),
                    self.surface_array,
            ...
            if terminal:
                ...
                plt.close()
                print(
                    "Last theta diff: " + str(theta_diff_list[len(theta_diff_list) - 1])
                )
                break
        if terminal == 0:
            plt.plot(np.arange(len(theta_diff_list)), theta_diff_list)
            ...
            plt.close()
            print("Last theta diff: " + str(theta_diff_list[len(theta_diff_list) - 1]))
